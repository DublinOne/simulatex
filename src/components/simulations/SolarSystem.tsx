...
const PLANETS = [
  { id: 'mercury', name: 'Mercury', color: '#b5b5b5', size: 12, orbit: 100, duration: 10, realOrbit: 58 },
  { id: 'venus', name: 'Venus', color: '#e39e1c', size: 18, orbit: 150, duration: 18, realOrbit: 108 },
  { id: 'earth', name: 'Earth', color: '#3f8cff', size: 20, orbit: 200, duration: 30, hasMoon: true, realOrbit: 150 },
  { id: 'mars', name: 'Mars', color: '#ff5733', size: 16, orbit: 250, duration: 50, realOrbit: 228 },
  { id: 'jupiter', name: 'Jupiter', color: '#d8ca9d', size: 40, orbit: 350, duration: 120, realOrbit: 778 },
  { id: 'saturn', name: 'Saturn', color: '#e3e0c0', size: 36, orbit: 420, duration: 240, hasRing: true, realOrbit: 1430 },
  { id: 'uranus', name: 'Uranus', color: '#b5e3e3', size: 26, orbit: 480, duration: 480, realOrbit: 2870 },
  { id: 'neptune', name: 'Neptune', color: '#3b5bdb', size: 26, orbit: 540, duration: 900, realOrbit: 4500 },
  { id: 'pluto', name: 'Pluto', color: '#aaa', size: 10, orbit: 600, duration: 1400, realOrbit: 5900 },
]

export function SolarSystem({ isPlaying, speed, showOrbits, showLabels, isTrueScale }: SolarSystemProps) {
  return (
    <div className={cn(
      "relative perspective-[2000px] flex items-center justify-center transition-all duration-1000",
      isTrueScale ? "w-[300vmin] h-[300vmin]" : "w-[90vmin] h-[90vmin]"
    )}>
      ...
      >
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
          <div className={cn(
            "rounded-full bg-gradient-to-br from-yellow-300 via-orange-500 to-red-600 shadow-[0_0_80px_20px_rgba(255,165,0,0.6)] animate-pulse transition-all duration-1000",
            isTrueScale ? "w-[120px] h-[120px]" : "w-[60px] h-[60px]"
          )} />
          {showLabels && (
        ...
        <ul className="list-none m-0 p-0 transform-style-3d">
          {PLANETS.map((planet) => {
            ...
            const orbitSize = isTrueScale ? (planet.realOrbit / 10) + 100 : planet.orbit

            return (
              <li 
                key={planet.id}
                className={cn(
                  "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/[0.03] transition-all duration-1000 transform-style-3d",
